...
void BArvore::divide_filho(No *x, int i)
{
    ...
    if (not y->folha)
        memcpy(z->filho, y->filho + t, t * sizeof(No *)); // copia t últimos filhos de y para z
...
{ // agrupa em filho[i] (=y) seu conteúdo original + chave[i] (=k) + o conteúdo de filho[i+1] (=z)
    No *y = x->filho[i];
    No *z = x->filho[i + 1];
...
bool BArvore::remove(No *x, int k)
{
    ...
    else
    {
        ...
        if (x->filho[i]->n == t - 1)
        {
            ...
            if (esq != NULL and esq->n >= t)
            {
                printf("CASO 3a esq (k=%d)...", k); // esquerdo
            ...
            else if (dir != NULL and dir->n >= t)
            {
                printf("CASO 3a dir (k=%d)...", k); // direito
...
void BArvorePrinter::visit(No const *node, unsigned level, unsigned child_index)
{
    ...
    {                                                   // non-leaf -> do all children so that .text_end for the right-most child becomes known
